...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        self.saver = tf.train.Saver(self.e_params, max_to_keep=5)
    ...
    def build_net(self):
        ...
        def attention(x, ch):
            ...
            with tf.variable_scope('convf'):
                w_convf = tf.get_variable('w_convf',[1,1,ch,ch//8],initializer=w_init,collections=name)
                b_convf = tf.get_variable('b_convf',[ch//8],initializer=b_init,collections=name)
                convf = conv2d(x, w_convf, 1)
                f = tf.nn.relu(tf.nn.bias_add(convf, b_convf))
                # (-1, 7, 7, 8)

            with tf.variable_scope('convg'):
                w_convg = tf.get_variable('w_convg',[1,1,ch,ch//8],initializer=w_init,collections=name)
                b_convg = tf.get_variable('b_convg',[ch//8],initializer=b_init,collections=name)
                convg = conv2d(x, w_convg, 1)
                g = tf.nn.relu(tf.nn.bias_add(convg, b_convg))
                # (-1, 7, 7, 8)

            with tf.variable_scope('convh'):
                w_convh = tf.get_variable('w_convh',[1,1,ch,ch],initializer=w_init,collections=name)
                b_convh = tf.get_variable('b_convh',[ch],initializer=b_init,collections=name)
                convh = conv2d(x, w_convh, 1)
                h = tf.nn.relu(tf.nn.bias_add(convh, b_convh))
                # (-1, 7, 7, 64)

            # N = h * w
            s = tf.matmul(at_flatten(g), at_flatten(f), transpose_b=True) # # [bs, N, N]

            beta = tf.nn.softmax(s)  # attention map

            o = tf.matmul(beta, at_flatten(h)) # [bs, N, C]
            gamma = tf.get_variable("gamma", [1], initializer=tf.constant_initializer(0.0))

            o = tf.reshape(o, shape=[-1, 7, 7, 64]) # [bs, h, w, C]
            x = gamma * o + x
        ...
        def build_layers(s, name):
            ...
            with tf.variable_scope('attention'):
                if self.args.attention:
                    x = tf.reshape(h_conv3, [-1, 7, 7, 64])
                    x_prime = attention(x, 64)
                    h_conv3_flat = tf.reshape(x_prime, [-1,3136])
                else:
